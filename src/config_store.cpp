...
bool config_load()
{
    ...
    if (!SPIFFS.exists(CFG_PATH)) {
        ...
        CONFIG.nbiot_enabled   = true;
        CONFIG.nbiot_apn       = "iot.1nce.net";
        CONFIG.nbiot_mqtt_host = "";
        CONFIG.nbiot_mqtt_port = 1883;
    ...
    // WIFI + ADMIN
    // ---------------------------------------------------------
    CONFIG.sta_ssid = doc["sta_ssid"] | "";
    CONFIG.sta_pass = doc["sta_pass"] | "";
    CONFIG.scale_cal_factor = doc["scale_cal_factor"] | 1.0f;

    CONFIG.admin_user = doc["admin_user"] | "admin";
    CONFIG.admin_pass = doc["admin_pass"] | "admin";

    // ---------------------------------------------------------
    // EXTERNAL MQTT
    // ---------------------------------------------------------
    CONFIG.ext_mqtt_host = doc["ext_mqtt_host"] | "";
    CONFIG.ext_mqtt_port = doc["ext_mqtt_port"] | 1883;
    CONFIG.ext_mqtt_user = doc["ext_mqtt_user"] | "";
    CONFIG.ext_mqtt_pass = doc["ext_mqtt_pass"] | "";
    ...
    CONFIG.mqtt_bridge_prefix_internal = doc["mqtt_bridge_prefix_internal"] | "local/";
    CONFIG.mqtt_bridge_prefix_external = doc["mqtt_bridge_prefix_external"] | "cloud/";
    ...
    CONFIG.nbiot_apn       = doc["nbiot_apn"]       | "iot.1nce.net";
    CONFIG.nbiot_mqtt_host = doc["nbiot_mqtt_host"] | "";
    CONFIG.nbiot_mqtt_port = doc["nbiot_mqtt_port"] | 1883;
    CONFIG.nbiot_mqtt_user = doc["nbiot_mqtt_user"] | "";
    CONFIG.nbiot_mqtt_pass = doc["nbiot_mqtt_pass"] | "";
